...
def train_model(classifier, feature_vector_train, label, feature_vector_valid, valid_y, is_neural_net=False):
    """
    This function will train and return predictions using the training data and testing data we have created earlier.
...
def dataset_prep():
    """
    A function that prepares the corpus so that it becomes usable. It also creates labels so that we don't lose track of which author
    is who.
    Returns a data frame imported from pandas.
    """
...
def split_section(data, n):
    """
    In this function, we accept a data and depending on what n is, we are splitting a specific feature from the data.
    returns train and test sets depending on what n is.
    """
...
def count_vector(train_x, valid_x, train_y, valid_y, trainDF):
    """
    This function will countvectorise the required texts. It will also use classifiers to see the accuracy
    This function will return an accuracy score of different classifiers.
    """
...
def tf_idf_vectors(train_x, valid_x, train_y, valid_y, trainDF):
    """
    Perfoms similar function as the count_vector but performs tfidf for word level and ngrams on the given input texts
    returns accuracy scores for different classifiers.
    """
...
def nlp_train(train_x, valid_x, train_y, valid_y):
    """
    Performs classifiers on different features we have extracted
    returns a score for each classifiers. Unfortunately we aren't able to use Xgboost here because of change in the data structure.
    """
    train_x1 = []
...
def nlp_features(trainDF):
    """
    This function does more feature extraction.
    https://www.analyticsvidhya.com/blog/2018/04/a-comprehensive-guide-to-understand-and-implement-text-classification-in-python/
    gives us an idea on how we can extract character counts, word counts, word density, punctuation counts, title word counts
    and uppercase word counts.
    returns an updated list that has all the features mentioned above.
    """
...
if __name__ == "__main__":
    ...
    output_file_nlp('test_results/5author_result2.txt', accuracy, accuracy1, accuracy2, accuracy3)
    
    train_x, valid_x, train_y, valid_y = split_section(trainDF, 2)
    accuracy, accuracy1, accuracy2, accuracy3 = nlp_train(train_x, valid_x, train_y, valid_y)
    output_file_nlp('test_results/5author_result3.txt', accuracy, accuracy1, accuracy2, accuracy3)
    
    train_x, valid_x, train_y, valid_y = split_section(trainDF, 3)
    accuracy, accuracy1, accuracy2, accuracy3 = nlp_train(train_x, valid_x, train_y, valid_y)
    output_file_nlp('test_results/5author_result4.txt', accuracy, accuracy1, accuracy2, accuracy3)
    
    train_x, valid_x, train_y, valid_y = split_section(trainDF, 4)
    accuracy, accuracy1, accuracy2, accuracy3 = nlp_train(train_x, valid_x, train_y, valid_y)
    output_file_nlp('test_results/5author_result5.txt', accuracy, accuracy1, accuracy2, accuracy3)
    
    train_x, valid_x, train_y, valid_y = split_section(trainDF, 5)
    accuracy, accuracy1, accuracy2, accuracy3 = nlp_train(train_x, valid_x, train_y, valid_y)
    output_file_nlp('test_results/5author_result6.txt', accuracy, accuracy1, accuracy2, accuracy3)
    
    train_x, valid_x, train_y, valid_y = split_section(trainDF, 6)
    accuracy, accuracy1, accuracy2, accuracy3 = nlp_train(train_x, valid_x, train_y, valid_y)
    output_file_nlp('test_results/5author_result7.txt', accuracy, accuracy1, accuracy2, accuracy3)