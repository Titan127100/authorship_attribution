...
def preprocessing(doc):
    ...
    return average_list, total_list, filtered_list, num_stopwords_l, num_punct_l
...
def average_sentence_length(doc):
    ...
    if len(doc) == 1:
        average_sent_length = doc[0]
        #print(average_length)
        return average_length
    for i in range(1,len(doc)):
        average_sent_length += (doc[i]-doc[i-1])
    average_sent_length = average_sent_length/(len(doc)-1)
    #print(average_length)
    total_sent = len(doc)
    return average_sent_length, total_sent
...
if __name__ == "__main__":
    ...
    #average_list, total_list, filtered_list, num_stopwords_l, num_punct_l = preprocessing(data)
